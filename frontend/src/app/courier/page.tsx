...
export default function CourierPage() {
  ...
  const handleStatusChange = async (orderId: string, newStatus: string) => {
    if (![OrderStatus.IN_DELIVERY, OrderStatus.DELIVERED].includes(newStatus as OrderStatus)) {
      return;
    }
  ...
  const activeOrders = orders.filter(
    order => [OrderStatus.ASSIGNED_TO_COURIER, OrderStatus.IN_DELIVERY].includes(order.status)
  );
        ...
        {orders.length === 0 ? (
          ...
          orders.map((order) => (
            <OrderCard
              ...
              showActions={[OrderStatus.ASSIGNED_TO_COURIER, OrderStatus.IN_DELIVERY].includes(order.status)}
            />
          ))
        )}
      </div>
